...
for file_article_md in $folder_markdown/*; do
    ...
    python3 $script_insert_code $file_article_md $folder_coq $folder_lean | \
...
python3 $script_generate_index $file_general_template $folder_markdown $file_coq_project $file_lean_project > $root_folder/index.html
