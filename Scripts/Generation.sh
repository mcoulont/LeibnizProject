...
for file_article_md in $folder_markdown/*; do
    ...
    python3 $script_manage_common_html $basename_without_extension $file_template_link_index $file_template_links_provers $file_template_switch_prover $file_template_switch_visibility_math_code $file_template_image_rocq $file_template_image_lean > $folder_html/$basename_without_extension.html
done
