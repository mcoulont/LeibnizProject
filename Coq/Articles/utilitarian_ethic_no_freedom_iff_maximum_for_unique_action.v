...
(p : maximizes ethic uf) (state : State) :=
...
(ethic : Ethic) (uf : @UtilityFunction ps) (p : maximizes ethic uf) :=
  forall (state: State), utilitarian_ethic_unique_maximum ethic uf p state.

Proposition utilitarian_ethic_no_freedom_iff_maximum_for_unique_action
{ps : PreferenceSpace} :
  forall (ethic: Ethic) (uf : UtilityFunction) (p : maximizes ethic uf) (state: State),
    @utilitarian_ethic_unique_maximum ps ethic uf p state <->
    leaves_no_freedom ethic state.
Proof.
  intros.
  unfold maximizes in p.
  split.
  - intro. unfold leaves_no_freedom. unfold unique.
    ...
    exists x. split.
    + pose proof (p state). pose proof (H1 x).
    ...
    + intros.
      ...
      pose proof (p state). pose proof (H3 x').
...
{ps : PreferenceSpace} :
  forall (ethic: Ethic) (uf : UtilityFunction) (p : maximizes ethic uf),
    @utilitarian_ethic_always_unique_maximum ps ethic uf p <-> never_leaves_freedom ethic.
Proof.
